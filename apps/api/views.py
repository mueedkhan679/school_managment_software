...
class StudentFeeStatementPDFView(APIView):
    """GET /api/v1/students/fees/statement/
    Generates a PDF fee statement for the logged-in student.
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not student:
            return Response(
                {"status": "error", "message": "Active student profile not found."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        details = [
            ...
            ["Class:", student.school_class.name, "Status:", student.get_status_display()],
        ]
        
        if student.admission_fee is not None:
            details.append(["Admission Fee:", f"Rs {student.admission_fee}", "", ""])
            
        t_details = Table(details, colWidths=[1.2*inch, 2.5*inch, 1.2*inch, 2.5*inch])
        ...
        total_paid = paid_fees.aggregate(total=models.Sum("amount"))["total"] or Decimal("0.00")
        curr_year_paid = paid_fees.filter(fee_year=current_year).aggregate(total=models.Sum("amount"))["total"] or Decimal("0.00")
        yearly_expected = student.yearly_fee
        yearly_pending = max(Decimal("0.00"), yearly_expected - curr_year_paid)

        summary_data = [
            ["Total Paid (All Time):", f"Rs {total_paid}"],
            ["Current Year Paid:", f"Rs {curr_year_paid}"],
            ["Current Year Pending:", f"Rs {yearly_pending}"],
        ]
        ...
        # Fee History Table
        elements.append(Paragraph("Fee History", styles['Heading3']))
        history_data = [["Date", "Month/Year", "Session", "Class", "Amount", "Ref"]]
        
        for fee in student.fees.order_by("-fee_year", "-fee_month", "-payment_date"):
            history_data.append([
                fee.payment_date.strftime("%Y-%m-%d"),
                f"{fee.get_fee_month_display()} {fee.fee_year}",
                fee.session_year or "-",
                fee.school_class.name if fee.school_class else "-",
                f"Rs {fee.amount}",
                fee.reference or "N/A"
            ])

        t_history = Table(history_data, colWidths=[1*inch, 1.3*inch, 1.2*inch, 1.2*inch, 1*inch, 1.5*inch])
